pipeline { 
    ...
    stages { 
            ...
            steps { 
                withCredentials([usernamePassword(credentialsId: 'credendials', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) { 
                    sh ''' 
                        ...
                        docker tag chat $DOCKER_USER/chat:latest 
                        docker push $DOCKER_USER/chat:latest 
                    ''' 
                } 
            } 
            post { 
        ...
        stage("Remove Docker Image Locally") { 
            steps { 
                sh """ 
                docker rmi -f ${DOCKERHUB_USERNAME}/chat || true 
                docker rmi -f chat || true 
                """ 
            } 
            post { 
                success { 
        ...
        stage("Stop and Restart Container") { 
            steps { 
                ...
                docker run -d --name app -p 8081:8080 ${DOCKERHUB_USERNAME}/chat 
                """ 
            } 
            post { 
                success { 
